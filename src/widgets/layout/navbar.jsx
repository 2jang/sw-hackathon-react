...
export function Navbar({ brandName, routes, action }) {
  ...
  const navList = (
    <ul className="mb-4 mt-2 flex flex-col gap-2 text-inherit lg:mb-0 lg:mt-0 lg:flex-row lg:gap-[60px]">
        ...
        >
          {href ? (
            <a
              href={href}
              target={target}
              className="flex items-center gap-1 p-1 font-bold text-[20xp]"
            >
              {icon &&
            ...
            <Link
              to={path}
              target={target}
              className="flex items-center gap-2 p-1 font-bold text-xl"
            >
              {icon &&
                React.createElement(icon, {
                  className: "w-[18px] h-[18px] opacity-75 mr-1",
